...
const DISABLE_EXT_DISPLAYS = _("Disables external displays.\nRequires restart.");
const ENABLE_FOR_EXT_DISPLAYS = _("Enable for external displays.\nRequires restart.");
const REQUIRES_RESTART = _("Requires restart.");
...
var PopDialog = GObject.registerClass(class PopDialog extends ModalDialog.ModalDialog {
...
var Ext = class Ext {
    constructor() {
        ...
        try {
            if (this.bus.GetSwitchableSync() == "true") {
                ...
                let compute_text = null, hybrid_text = null, integrated_text = null, nvidia_text = null;
                ...
                this.bus.connectSignal("HotPlugDetect", (proxy) => {
                    if (this.graphics_profiles) {
                        log("hotplug event detected");
                        let graphics = proxy.GetGraphicsSync();
                        let current = null;
                        if (graphics == "compute") {
                            current = "Compute";
                        }
        ...
        this.bus.connectSignal("PowerProfileSwitch", (_proxy, _sender, [profile]) => {
    ...
    set_graphics_profile_ornament(graphics_profiles, graphics) {
    ...
    set_power_profile_ornament(active_profile) {
        this.reset_profile_ornament();
        let obj = null;
    ...
    hotplug(current, item, name, vendor) {
        if (this.switched || this.notified) {
            return;
        }
        this.notified = true;
        let dialog = new PopDialog("video-display-symbolic", _("Switch to ") + name + GRAPHICS + _(" to use external displays"), _("External displays are connected to the NVIDIA card. Switch to ") + name + _(" graphics to use them."));
        dialog.open();
        dialog.setButtons([{
                action: () => {
                    dialog.close();
    ...
    graphics_activate(item, name, vendor) {
        this.switched = true;
        if (!item.setting) {
            item.setting = true;
            let dialog = new PopDialog("dialog-warning-symbolic", _("Preparing to Switch to ") + name + GRAPHICS, name + _(" graphics will be enabled on the next restart"));
            dialog.open();
            this.bus.SetGraphicsRemote(vendor, (_result, error) => {
                item.setting = false;
    ...
    reset_graphics_ornament(graphics_profiles) {
